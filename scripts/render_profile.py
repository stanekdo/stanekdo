"""Render the profile from authenticated aggregate GitHub statistics."""
...
def load_profile(assets: Path) -> Profile:
    ...
    for value in [
        *stats.values(),
        ...
        if type(value) is not int or value < 0:
            raise RenderError(
                "The profile snapshot contains an invalid statistic count."
            )
    created, today = (
        date.fromisoformat(snapshot["created"]),
    ...
    def period(value):
        ...
        if not created <= start <= end <= today:
            ...
            if (
                not created.toordinal() - 6
                <= start.toordinal()
                <= end.toordinal()
                <= today.toordinal()
            ):
                raise RenderError(
                    "The profile snapshot contains an invalid streak period."
                )
    ...
    streak = [
        f"{streak_data['total']:,}",
        "Total contributions",
        display_date(snapshot["created"]) + " - Present",
        "Current week streak",
        period(current),
        str(current["weeks"]),
    ]
    rows = [
        ...
        for key, label in (
            ...
            ("prs", "Total PRs"),
            ("issues", "Total Issues"),
            ("contributed_to", "Contributed to (last year)"),
        )
    ]
    ...
    for index, (name, score) in enumerate(trophy_scores(snapshot).items()):
        ...
        artwork = list(
            ET.fromstring(
                f'<svg xmlns="{NS[1:-1]}">{trophy_data["icons"][trophy_level]}</svg>'
            )
        )
        # Several source cards reuse gradient IDs. Give each trophy its own IDs.
        ids = {
            e.attrib["id"]: f"trophy-{index}-{e.attrib['id']}"
        ...
        trophies.append(
            Trophy(
                ...
                points,
                progress,
                "".join(ET.tostring(child, encoding="unicode") for child in artwork),
            )
        )
    ...
    for language in languages:
        if (
            not isinstance(language["name"], str)
            or not language["name"]
            or language["name"] in names
        ):
            raise RenderError("Language names must be nonempty and unique.")
        names.add(language["name"])
        if (
            type(language["percentage"]) not in (int, float)
            or not 0 <= language["percentage"] <= 100
            or not re.fullmatch(r"#[0-9a-fA-F]{6}", language["color"])
        ):
    ...
    if (
        len(languages) > 8
        or abs(sum(language["percentage"] for language in languages) - 100) > 0.0001
    ):
        raise RenderError(
            "The language snapshot must contain up to eight shares totaling 100 percent."
        )
    languages = sorted(
        languages, key=lambda language: (-language["percentage"], language["name"])
    )
...
class Svg:
    ...
    def text(
        self, x, y, value, size=14, color="fg", weight=400, anchor="start", mono=False
    ) -> str:
        font = (
            "Consolas,monospace"
            if mono
            else "-apple-system,BlinkMacSystemFont,Segoe UI,Arial,sans-serif"
        )
...
def place(x, y, content) -> str:
    return f'<g transform="translate({x} {y}) scale(1)">{content}</g>'


def streak_card(s: Svg, profile: Profile) -> str:
    v = profile.streak
    body = s.ring(85, 76, 52, 1, 2) + s.text(85, 84, v[5], 38, "accent", 500, "middle")
    body += s.text(85, 102, "WEEKS", 9, "muted", 500, "middle")
    body += s.text(166, 69, "Current week streak", 15, weight=600) + s.text(
        166, 94, v[4], 11, "muted"
    )
...
def stats_card(s: Svg, profile: Profile) -> str:
    body = s.box(440, 238) + s.text(22, 34, "GitHub stats", 16, weight=600)
    for i, (label, value) in enumerate(profile.stats):
        y = 75 + i * 31
        body += s.text(22, y, label.rstrip(":"), 13, "muted")
        body += s.text(276, y, value, 14, weight=600, anchor="end", mono=True)
...
def language_card(s: Svg, profile: Profile) -> str:
    ...
    for i, language in enumerate(profile.languages):
        ...
        body += s.text(
            x + 13, y + 13, f"{language['percentage']:.2f}%", 11, weight=500, mono=True
        )
    return body
...
def trophy_card(s: Svg, trophy: Trophy) -> str:
    ...
    body += s.text(82, 27, trophy.name, 10.5, weight=600) + s.text(
        82, 47, trophy.points, 11, "muted", mono=True
    )
...
def render(profile: Profile, theme: str) -> str:
    s = Svg(theme)
    width = 780
    body = streak_card(s, profile)
    body += place(0, 176, stats_card(s, profile))
    body += place(450, 176, language_card(s, profile))
    trophy_y = 460
    body += s.text(0, trophy_y - 18, "Trophies", 14, weight=600)
    ranked = [t for t in profile.trophies if t.rank != "?"]
    columns, step_x, step_y = 4, 200, 116
    for i, trophy in enumerate(ranked):
        row, column = divmod(i, columns)
        body += place(column * step_x, trophy_y + row * step_y, trophy_card(s, trophy))
    ...
    description += (
        ". ".join(f"{label} {value}" for label, value in profile.stats)
        + f". Rank {profile.rank}. "
    )
...
def render_files(assets: Path, output: Path) -> None:
    ...
    rendered = {f"profile-{theme}.svg": render(profile, theme) for theme in THEMES}
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print("Rendered profile for dark and light themes.")
    return 0
